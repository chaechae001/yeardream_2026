...
hf auth whoami  <- 현재 로그인 계정 조회
hf download [model명] --local-dir [경로]   <- 모델 다운로드

토큰과 브라우저 중 택 1 하라고 함
만약 브라우저에 허깅페이스가 로그인 되어있으면, 시스템 브라우저로 로그인 됨
