...
# device = [0:GPU, -1:CPT(default), "cuda": 사용가능한 GPU]
...
# 결과값은 리스트 안의 딕셔너리 형태로 나오기 때문에, 첫번째 요소[0]의 'summary_text' 키값을 불러와 출력
...
early_stopping (bool) : 기본이 False
True : 설정한 상위 후보 만큼 모이면 더이상 계산하지 않고 중단
False : 설정한 상위 후보 만큼 모인다해도 계속 계산 (더 높은 점수를 만날 수 있으므로..)
